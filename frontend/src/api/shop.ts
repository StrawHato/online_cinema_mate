...
import type { Cart, Order, Page, Payment } from '../types'
...
export const createCheckout = async (uuid: string) => (await api.post<{ checkout_url: string; payment_uuid: string }>(`/payments/${uuid}/checkout/`)).data
export const getOrders = async () => (await api.get<Page<Order>>('/orders/')).data
export const getPayment = async (uuid: string) => (await api.get<Payment>(`/payments/${uuid}/`)).data
